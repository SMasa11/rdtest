...
#' @param bool.chisqStd boolean option for using standardized t stat to conduct L2 test
#'
#' @export
####

returnResultsJointTest <- function(df.data,int.dimZ,int.J=3,bool.maxTest=FALSE,bool.maxTestVinv=FALSE,bool.knownV=FALSE,real.covZ=NULL,fun.adjustBand=NULL,bool.chisqStd=FALSE)
{
  library("rdrobust")
  library("rddensity")
  library("R.utils")
  ...
  if (bool.maxTest == FALSE) {
    list.resultCovariates <- computeStatChiSquared(df.data=df.data,int.dimZ=int.dimZ,bool.joint=TRUE,int.J=int.J,bool.knownV=bool.knownV,real.covZ=real.covZ,fun.adjustBand=fun.adjustBand,bool.chisqStd=bool.chisqStd)
  } else {
  ...
  if (bool.maxTest==FALSE) {
    if (bool.chisqStd) {
      real.statChisqStdJoint <- list.resultCovariates$real.statChiSq + real.statTX^2
      ...
      for (l in seq(1:int.numSimulDraws)) {
        ...
        vec.randomDrawStats[l] <- sum((mat.CorSqrt %*% mat.randomNorm[,l])^2)
        real.pvalue <- real.pvalue + (vec.randomDrawStats[l] >= real.statChisqStdJoint)/int.numSimulDraws
      }
      bool.rejectJointNull <- (real.pvalue <= 0.05)
      real.criticalValueJoint <- quantile(vec.randomDrawStats,0.95)
    } else {
      bool.rejectJointNull <- (list.resultCovariates$real.statChiSq + real.statTX^2 >= qchisq(.95, df=(int.dimZ+1)))
    }
    ...
    if (bool.maxTestVinv == TRUE) {
      ...
      bool.rejectJointNull <- (real.statMaxJoint >= qchisq(0.95^(1/(int.dimZ+1)),1))
      real.criticalValueJoint <- qchisq(0.95^(1/(int.dimZ+1)),1)
    } else {
      real.statMaxJoint <- max(list.resultCovariates$real.statMax,real.statTX^2)
      vec.randomDrawStats <- rep(0,int.numSimulDraws)
      #Generates the same random normal vector with the seed of 373568, restoring the currently loading seed
