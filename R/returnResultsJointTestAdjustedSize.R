...
returnResultsJointTestAdjustedSize <- function(df.data,int.dimZ,int.J=3,bool.maxTest=FALSE,bool.maxTestVinv=FALSE,bool.knownV=FALSE,real.covZ=NULL,fun.adjustBand=NULL,vec.critRange=vec.critRange)
{
  library("rdrobust")
  library("rddensity")
  library("R.utils")
  ...
  vec.rejectJointNull <- rep(0,length(vec.critRange))
  counter <- 1
  for (real.critP in vec.critRange) {
    if (bool.maxTest==FALSE) {
      bool.rejectJointNull <- (list.resultCovariates$real.statChiSq + real.statTX^2 >= qchisq(real.critP, df=(int.dimZ+1)))
    } else {
      if (bool.maxTestVinv == TRUE) {
        # standardized max stats
        real.statMaxJoint <- max(list.resultCovariates$real.statMax,real.statTX^2)
        bool.rejectJointNull <- (real.statMaxJoint >= qchisq(real.critP^(1/(int.dimZ+1)),1))
        real.criticalValueJoint <- qchisq(real.critP^(1/(int.dimZ+1)),1)
      } else {
        real.statMaxJoint <- max(list.resultCovariates$real.statMax,real.statTX^2)
        ...
        bool.rejectJointNull <- (real.pvalue <= (1-real.critP))
        real.criticalValueJoint <- quantile(vec.randomDrawStats,real.critP)
      }
    }
    vec.rejectJointNull[counter] <- bool.rejectJointNull
    counter <- counter + 1
  }

  return(vec.rejectJointNull)
}
