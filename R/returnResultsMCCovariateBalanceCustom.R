...
returnResultsMCCovariateBalance <- function(jump,dim,n,cov.z,frac.jump = 0,bool.mutePrint=FALSE,bool.maxTest=FALSE)
{
  library("rdrobust")
  library("rddensity")
  library("ggplot2")
  ...
  for (loop in 1:ns)
  {
    data <- RDTests::simulateCustomDGPs(int.typeDGP = 1, int.dim = dim, int.sampleSize = n, real.TE = jump,real.corr = cov.z)
    list.resultCovariateBalance <- returnResultsBalanceTest(df.data=data,int.dimZ=dim,bool.maxTest=bool.maxTest)
    ...
    if (bool.maxTest == FALSE) {
      ...
      vec.statMaxJoint[loop]  <- list.resultCovariateBalance$real.statMaxJoint
    }
  ...
  if (!bool.mutePrint) {
    ...
    message(joint.num.reject/ns)
  }
  summary <- list(chi.stat.vec=chi.stat.vec,vec.statMaxJoint=vec.statMaxJoint,effN.mean = mean(effN.z.vec),cor.z=mean(covZ1Z2.vec),naive=naive.num.reject/ns,bonfe=bonfe.num.reject/ns,joint=joint.num.reject/ns)
  return(summary)
}
